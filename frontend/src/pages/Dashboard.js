...
function Dashboard() {
  ...
  useEffect(() => {
    fetch('http://localhost:8080/api/dashboard/stats').then(r=>r.json()).then(setStats).catch(()=>{});
    fetch('http://localhost:8080/api/students').then(r=>r.json()).then(d=>setStudents(d.slice(0,5))).catch(()=>{});
    fetch('http://localhost:8080/api/activities').then(r=>r.json()).then(d=>setActivities(d.slice(0,5))).catch(()=>{});
  }, []);
