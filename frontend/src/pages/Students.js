...
function Students() {
  ...
  const load = () => {
    fetch('http://localhost:8080/api/students', { headers: headers() })
  ...
  const handleSave = () => {
    ...
    fetch(url, { method, headers: headers(), body: JSON.stringify(form) })
      .then(r => r.json())
      .then(() => { setForm(EMPTY); setShowForm(false); setEditId(null); setLoading(false); load(); })
      .catch(e => { alert('Error: ' + e.message); setLoading(false); });
  };

  const handleEdit = (s) => {
    setForm({ name:s.name, rollNumber:s.rollNumber, department:s.department||'',
      year:s.year||'', email:s.email||'', phone:s.phone||'' });
    setEditId(s.id);
    setShowForm(true);
  ...
  return (
    <div className="fade-in">
      <div className="page-hero">
        ...
        <button className="add-button" onClick={() => { setShowForm(!showForm); setEditId(null); setForm(EMPTY); }}>
          + Add Student
        </button>
      </div>
      ...
      {showForm && (
        <div className="form-panel">
          <div className="form-panel-hdr">
            <span className="form-panel-title">{editId ? '✏️ Edit Student' : '➕ Add New Student'}</span>
              ...
              {filtered.map((s,i) => (
                <tr key={s.id}>
                  ...
                  <td>
                    ...
                    <div className="td-phone">{s.phone}</div>
                  </td>
                  <td>
                    <div className="action-group">
                      <button className="btn-view-sm" onClick={() => navigate(`/students/${s.id}`)}>View</button>
                      <button className="btn-edit-sm" onClick={() => handleEdit(s)}>Edit</button>
