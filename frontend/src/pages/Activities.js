...
function Activities() {
  ...
  const load = () => {
    fetch('http://localhost:8080/api/activities').then(r=>r.json()).then(setActivities).catch(()=>{});
  };
  useEffect(() => {
    load();
    fetch('http://localhost:8080/api/students').then(r=>r.json()).then(setStudents).catch(()=>{});
  }, []);
  ...
  const handleSave = () => {
    ...
    fetch('http://localhost:8080/api/activities', {
      ...
      body: JSON.stringify({...form, studentId:parseInt(form.studentId)})
    })
    .then(r=>r.json())
  ...
  const handleDelete = (id) => {
    ...
    fetch(`http://localhost:8080/api/activities/${id}`,{method:'DELETE'}).then(()=>load()).catch(()=>{});
  };
